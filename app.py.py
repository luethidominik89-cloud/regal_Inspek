...
if selected_customer == "---":
    ...
    with tab1:
        ...
        with col1:
            ...
            regal_nr = st.text_input("Regal-Nr.", value=r_val, key=f"r_{it}")
            ...
            pos = st.text_input("Position", value=p_val, key=f"p_{it}")
        ...
        with col2:
            ...
            m_list = ["Stapleranprall", "Stift fehlt", "Anker lose", "Überladung", "Sonstiges"]
            m_val = edit["Mangel"].split(":")[0] if edit else "Stapleranprall"
            m_idx = m_list.index(m_val) if m_val in m_list else 0
            mangel_sel = st.selectbox("Mangel", m_list, index=m_idx, key=f"m_{it}")
            
            k_val = edit["Mangel"].split(":")[1].strip() if edit and ":" in edit["Mangel"] else ""
            komm = st.text_input("Zusatz-Info", value=k_val, key=f"k_{it}")
            
            ms_list = ["Beobachten", "Tausch 4 Wo.", "SOFORT SPERREN", "Anker nachziehen"]
        ...
        if st.session_state.inspections:
            ...
            if st.button("↩️ Letzten Schritt rückgängig (Undo)", use_container_width=True):
    ...
    with tab2:
        # Archiv (wie bisher, ohne Inhaltsverzeichnis im PDF)
